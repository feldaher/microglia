...
threshold_vector=5.0 # keep only neuron with a significant displacement
...
for i in nPts:
	if i>0:# Filtering the track with enough points
		if dataPoints["CID"][index]>1: # Filtering tracks to only keep the ones near the injury (CID=2)
...
fig, ax = plt.subplots()
q = ax.quiver(x1,y1,u,v,units='width')
plt.show()
