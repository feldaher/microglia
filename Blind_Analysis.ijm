...
Aim: Blind filenames for image analysis
Input: a folder containing the tiff or czi files to be blinded
Output: folder containing the tiff blinded files and a log.txt file containing the association between new names and original ones.
The original name also appears in the metadata of the blinded file.
...
macro "Blind Analysis" {
	...
	for (i = 0; i < allNames.length; i ++) {
		if (endsWith(allNames[i], ".tif")||endsWith(allNames[i], ".czi")) {
