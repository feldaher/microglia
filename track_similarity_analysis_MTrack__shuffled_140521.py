...
for index in range(nbNeurons):
	##### SHUFFLING points order of neuron trajectories			
	random.shuffle(trajecListNeurons[index])
...
#		print(index)
	...
	df.to_csv(directory+'/'+truncate_filename[0]+'_'+'microglia'+str(indexMicroglia)+'_'+similMetric+'_Trajectory_distance_shuffled.csv', index = False, header=True)
